...
The map originally carried hand-approximated coordinates and looked like a rough sketch. It now uses real boundaries, generated by `scripts/generate-map-paths.mjs` into `components/canadaGeography.ts`:

```bash
node scripts/generate-map-paths.mjs
```

- **Projection:** Lambert Conformal Conic, standard parallels 49°N/77°N, central meridian 91.87°W — what Statistics Canada uses for national maps.
- **Source:** `click_that_hood` canada.geojson (public domain, derived from Natural Earth). Cached to `scripts/.cache-canada.geojson`, which is gitignored and refetched on demand.
- **Size:** 72.5 KB raw / **27.7 KB gzipped**, tuned via two knobs, both overridable by env var:
  - `MIN_RING_AREA` (16px²) drops islands too small to see. Safe to raise — islands share no borders.
  - `SIMPLIFY_TOLERANCE` (0.6px) is Douglas–Peucker vertex reduction. **Raise with care:** provinces are simplified independently, so a shared border can diverge by up to the tolerance. It's kept under the white inter-province stroke so any divergence is covered. (Most Canadian interprovincial borders are straight survey lines, which simplify with zero error.)
- Verified intact: Newfoundland island *and* Labrador as separate parts, Cape Breton, Vancouver Island, Haida Gwaii, and 35 Nunavut landmasses including Baffin, Victoria and Ellesmere.

**Labels.** Provinces get an inline abbreviation; Nova Scotia and PEI are too small (NS's centroid sits ~2.6px from a coastline, PEI renders at ~107px²) so they use callouts with leader lines.

⚠️ **The leader routes in `CALLOUTS` are computationally verified, not eyeballed.** Every natural left-to-right route cuts straight through Nova Scotia — the first attempt did exactly that. PEI has essentially one clean exit: due north through the ~13px channel between Newfoundland island (ends y≈465) and Cape Breton (starts y≈478), then east. If you move these points, re-run a segment-intersection check against the province rings rather than trusting the eye.
